...
document.addEventListener('DOMContentLoaded', async function () { // HTML과 script가 로드된 시점에 발생하는 이벤트.
    await makeFeedAjax(document.querySelector('#friend_pk').value, page_count).then((myJson) => {
      makeFeed(myJson)
    })
    page_count++
    await ajax()
function ajax() {
    ...
    makeFeedAjax(document.querySelector('#friend_pk').value, page_count).then((myJson) => {
      makeFeed(myJson)
    })
    page_count++
  }
